...
def _render_full_ui(controller=None):
    ...
    # Sidebar controls
    try:
        start_real, start_dry, _ = sidebar_controller.render_in(st.sidebar)
    except Exception:
        start_real, start_dry = False, False
    ...
    dash_left, dash_right = st.columns([1, 999], gap="small")
    ...
    try:
        ...
        try:
            ...
            # Fallback: render in the left dashboard column if sidebar unavailable
            try:
                render_theme_selector(ui=dash_left)
            except Exception:
                pass

    # Right: Status + Active bots
    with dash_right:
        with _safe_container(border=True):
            st.subheader("📋 Status")
    ...
    if start_real or start_dry:
        try:
            ...
            _merge_query_params({"bot": bot_id_started, "view": "monitor"})
            st.success(f"✅ Bot {bot_id_started[:8]} iniciado! Abrindo Monitor na mesma aba.")
            st.rerun()
        except Exception as e:
